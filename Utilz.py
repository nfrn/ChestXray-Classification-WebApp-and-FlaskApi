...
def getImages(img):
    print(img)
    return img_to_array(load_img(img, color_mode="grayscale", target_size=(224, 224))) / 255.
...
class ChestXRaySequence(Sequence):
    ...
    def get_batch_features(self, idx):
        ...
        imageFeatures = np.array([getImages(im) for im in self.x2[idx * self.batch_size: (1 + idx)
                                                                                        * self.batch_size]])
        textReport = self.x1[idx * self.batch_size: (idx + 1) * self.batch_size]
        arrayReport = []
        for report in textReport:
            arrayReport.append(np.array(report))
        return [imageFeatures, np.array(arrayReport)]
...
class ChestXRayImageSequence(Sequence):

    def __init__(self, x1,y, batch_size, mode='train'):
        self.x1 = x1
        self.y = y
        self.n = len(self.y)
        self.batch_size = batch_size
    ...
    def get_batch_features(self, idx):
        # Fetch a batch of inputs
        imageFeatures = np.array([getImages(im) for im in self.x1[idx * self.batch_size: (1 + idx)
                                                                                        * self.batch_size]])
    ...
    def getTotal(self):
        labels = self.y
        imageFeatures = np.array([getImages(im) for im in self.x1])
        return [imageFeatures], labels


class ChestXRayTextSequence(Sequence):
    ...
    def getTotal(self):
        ...
        for report in textReport:
            arrayReport.append(np.array(report))
        return np.array(arrayReport), labels